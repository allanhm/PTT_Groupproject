...
void StatusAfterBPlyaer_f(Players user); // declaration of the function

void StatusAFterBMonster_f(Monster enemy); // declaration of the function

void Battle_f(Players &user, Monster &enemy, int round, int userdeck[], int monsterdeck[]){ // This function illustrates how the battle is done each round. and decides on who wins or loses.
    ...
    while(keepfight_v) {
        ...
        if(current_potion.size() ==0 ){
            cout<< "There is no potion you can use" <<endl<<endl; // shows that there are no potion available 
        }
        else {
            cout << "Current Potion: ";
        ...
        if (userselectedcard_v > monster_card_v) { //user wins
            cout << "-----You won!-----" << endl;
            damage = userselectedcard_v - enemy.monsterdefense_v+current_weapon; // calculates the damage
        ...
        if (userselectedcard_v <= monster_card_v) { // monster wins
            cout << "-----You lost!-----" << endl;
            damage = monster_card_v - (user.playerdefense_v+current_armor); // calculates the damage
            if (damage <= 0) damage =0;
        ...
        StatusAfterBPlyaer_f(user); // Displays the status after the battle

        StatusAFterBMonster_f(enemy); // Displays the status after the battle
...
void StatusAfterBPlyaer_f(Players player){ // displays the current status of the user
